...
void sobel(cv::Mat &input, cv::Mat &processed);
...
int main(int argc, char *argv[]) {
    ...
    cv::Mat temp1, temp2;
    // convertColorToGray(input, temp1);
    //blur(input, processed);
    //sobel(input, processed);
    cv::threshold(input, processed, 100, 255, cv::THRESH_BINARY);
    ...
    cv::imwrite("processed_blur.jpg", processed);
...
void blur(cv::Mat &input, cv::Mat &processed) {
    #if FLAG // use built−in function
    cv::GaussianBlur(input, processed, cv::Size(5, 5), 3, 3);
...
void sobel(cv::Mat &input, cv::Mat &processed) {
    #if FLAG // use built−in function
    cv::Sobel(input, processed, CV_32F, 1, 1);
    #else
